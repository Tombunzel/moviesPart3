...
class MovieApp:
    ...
    def get_list_of_ratings(self):
        """
        create a list of all movie ratings
    ...
    @staticmethod
    def check_movie_in_data(movie, movies):
        """
        checks a user input movie name through the movies in data
        returns: None if no input was given and prints a message
                 False if no movie was found and prints a message
                 The correct movie name from storage if found
        """
        if not movie:
            print("\n\033[31mNothing entered,"
                  "sending you back to main menu\033[00m")
            return None
        movie_titles_lower = {movie.lower(): movie for movie in movies}
        if movie.lower() not in movie_titles_lower:
            print(f"\n\033[31mCouldn't find movie {movie}!\033[00m")
            return False

        correct_movie_title = movie_titles_lower[movie.lower()]
    ...
    def _command_add_movie(self):
        """
        uses the data_fetcher file to search and fetch a dict of movie info
        ...
        while True:
            ...
            try:
                ...
                if movie_dict['Type'] != 'movie':
                    print(f"\n\033[31mCouldn't find a movie called"
                          f"{movie_name}\033[00m")
                    break
                ...
                print("\033[31m\nConnection error,"
                      "please check your connection.\033[00m")
                break

            if user_input.lower() == 'y':
                if movie_dict['Type'] != 'movie':
                    print(f"\n\033[31m{title} is not a movie,"
                          f"but a {movie_dict['Type']}\033[00m")
                    break
    ...
    def _command_delete_movie(self):
        ...
        movies = self._storage.get_movies()
        while True:
            name = input("\033[00mEnter movie name to delete: \033[32m")
            print("\033[00m", end='')  # white
            correct_movie_name = self.check_movie_in_data(name, movies)
            if correct_movie_name is False:
                break
            if correct_movie_name is None:
                continue
            self._storage.delete_movie(correct_movie_name)
            print(f"Movie {correct_movie_name} successfully deleted")
            break

    def _command_update_movie(self):
        """
        Allows user to insert a note to a movie in data
        movie_titles_lower is a dict of lowercase titles
        referring to the correct titles
        """
        movies = self._storage.get_movies()
        while True:
            ...
            if correct_movie_name is None:
                continue
            user_note = input("Enter movie notes: \033[32m")
            print("\033[00m", end='')  # white
            self._storage.update_movie(correct_movie_name, user_note)
            print(f"\n\033[00mMovie {correct_movie_name} "
    ...
    def calc_avg_rating(list_of_ratings):
        ...
        for rating in list_of_ratings:
            sum_ratings += rating
        average_rating = sum_ratings / amount_of_movies
        return average_rating

    @staticmethod
    def calc_median_rating(movies, list_of_ratings):
        """
        calculates median rating depending on number of movies (odd or even)
        :return: the median rating
        """
        list_of_ratings = sorted(list_of_ratings, reverse=True)
        movies_length = len(movies)
        if movies_length % 2 == 0:
            left_median = list_of_ratings[int(movies_length / 2) - 1]
            right_median = list_of_ratings[int(movies_length / 2)]
            median = (left_median + right_median) / 2
            return round(median, 1)
        median = list_of_ratings[int(len(movies) / 2)]
        return round(median, 1)
    ...
    def _command_movies_by_rating(self):
        ...
        list_of_ratings = self.get_list_of_ratings()
        print()
        list_of_ratings = sorted(list(set(list_of_ratings)), reverse=True)
        for rating_in_list in list_of_ratings:
            for movie, infos in movies.items():
                rating = infos['rating']
                if rating_in_list == rating:
                    print(f"{movie} ({infos['year']}), {rating}")

    def _command_movies_by_year(self):
        """
        prints all movies ordered by year of publishing, order chosen by user
        """
        movies = self._storage.get_movies()
        list_of_years = []
        ...
        while True:
            ...
            if user_input.lower() == "o":
                is_reverse = False
                break
            print("\033[31m\nPlease enter either 'a' for ascending"
                  "or 'd' for descending\033[00m")
    ...
    def check_filter_input(user_input):
        ...
        try:
            ...
            print("\033[31m\nPlease enter a number or leave"
                  "blank for no filter\033[00m\n")
            return user_input
    ...
    def _command_filter_movies(self):
        ...
        while True:
            start_year = input("Enter start year (leave blank for"
                               "no start year): \033[32m")
            print("\033[00m", end="")
            if start_year == '':
                start_year = 1888
        ...
        while True:
            end_year = input("Enter end year (leave blank for"
                             "no end year): \033[32m")
            print("\033[00m", end="")
            if end_year == '':
                end_year = 2024
        ...
        print("\nFiltered movies:")
        for movie, info in movies.items():
            if (info['rating'] >= minimum_rating and
                    start_year <= info['year'] <= end_year):
                print(f"  {movie} ({info['year']}): {info['rating']}")

    @staticmethod
    def _read_template():
        """reads the template HTML file and returns its contents"""
        with open("_static/index_template.html", "r",
    ...
    def _create_html(self, template):
        """
        creates a new HTML file using
        the template and a dict of all movies
        """
        movies = self._storage.get_movies()
        site_title = "My Movies Site"
        with open("_static/index.html", "w",
                  encoding="UTF-8") as handle:
            output = template
